...
- 빌드된 이미지를 Kubernetes 클러스터에 배포하는 CD 파이프라인 구현
- GitHub Secrets를 활용한 민감 정보(Docker Hub 토큰, kubeconfig) 관리
...
- `CP_K8S_CONTEXT`: kubeconfig 내용 등록 (base64 인코딩 없이 원문)
- 기대 결과: Secrets 목록에 2개 항목 확인

### Step 2: 파이프라인 YAML 파일 복사
- 명령어: `cp ~/_Lecture_cicd_learning.kit/ch5/5.2/1.build-deploy-pipeline.yaml .github/workflows/1.build-deploy-pipeline.yaml`
- 기대 결과: `.github/workflows/` 디렉토리에 파이프라인 파일 생성

### Step 3: 코드 커밋 및 푸시
- 명령어: `git add . && git commit -m "cicd: add build and deploy actions" && git push origin main`
- 기대 결과: GitHub Actions가 자동으로 트리거됨

### Step 4: 파이프라인 실행 결과 확인
- GitHub → Actions 탭에서 파이프라인 실행 상태 확인
- 기대 결과: init-and-test → build → deploy → notify-success-result 순서로 실행 완료
...
| 단계 | 검증 방법 | 기대 결과 |
|------|----------|----------|
| Secrets 등록 | GitHub Settings → Secrets 확인 | DOCKERHUB_TOKEN, CP_K8S_CONTEXT 존재 |
...
- ✅ deploy_manifest/worklog-backend.yaml의 이미지 태그가 파이프라인에서 빌드한 태그와 일치하는지 확인한다.
