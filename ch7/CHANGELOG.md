...
**7.3 cleanup 경고 강화**
- 주의사항 ✅ → ⛔ 변경: "7.5/7.6 Rollout과 Deployment 이름 충돌" 사유 추가 명시
- 이유: cleanup 누락 시 Rollout 생성 시 `kind: Deployment`와 충돌 발생 가능성

**7.7 3-tier 전환 배경 섹션 신규 추가**
- 7.3~7.6에서 단일 서비스(worklog-backend)로 배포 전략 학습 → 7.7부터 3-tier 전체 스택으로 전환됨을 명시
- 이유: 갑작스런 아키텍처 변화로 학습자 혼란 가능. 전환 배경과 ch8/ch9와의 연결 관계를 GUARDRAIL 상단에 추가
